...
2. 逐檔比對本機與 repo 的所有同步檔案：
   ...
   - **.skill-lock.json**（`~/.agents/.skill-lock.json` ↔ `agents/.skill-lock.json`）：比對時忽略裝置特定欄位（`lastSelectedAgents`、`dismissed`），將兩邊 JSON 載入後移除這些欄位再比較。同步時仍複製完整檔案。
...
5. 若有差異：針對每個有差異的檔案顯示 diff（標明檔名），然後使用 AskUserQuestion 詢問策略，選項為：「1. 用本機設定覆蓋雲端」、「2. 用雲端設定覆蓋本機」、「3. 取消」
   - 若選「1. 用本機設定覆蓋雲端」：將本機的對應檔案複製到 repo 對應目錄（`claude/` 或 `agents/`），顯示 diff，再使用 AskUserQuestion 詢問「是否自動 commit 並 push？」，選項為「1. 自動 commit 並 push」、「2. 自行處理」
   ...
   - 若選「2. 用雲端設定覆蓋本機」：將 repo 版複製到本機。若更新的檔案包含 `.skill-lock.json`，複製完成後額外提示：「`.skill-lock.json` 已更新，如需還原 skills 請執行：`skills experimental_install`」
   - 若選「3. 取消」：不執行任何操作
