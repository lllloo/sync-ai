...
- **Global scope**: `D:/code/sync-ai/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/code/sync-ai/.claude/notes/`)
...
2. Glob `D:/code/sync-ai/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/code/sync-ai/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/code/sync-ai/.claude/notes/` — works in any directory
