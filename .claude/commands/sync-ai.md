...
```json
{
  ...
  "agents":      { "same": bool, "repoOnly": [], "localOnly": [], "conflicts": [] },
...
- `agents` / `commands`：`repoOnly` 在 repo 但本機缺少；`localOnly` 本機有但不在 repo；`conflicts` 兩邊都有但內容不同
...
對每個有差異的檔案，先顯示 diff，再使用 AskUserQuestion 詢問處理方式：
...
#### 1. 用 repo 版（建議）
- label: `用 repo 版`
- description: `以 claude/ 的版本覆蓋本機`
- 動作：執行 `node .claude/commands/sync-ai-apply.js --action write-local --file <fileKey>` 將 repo 版寫入本機

#### 2. 用本機版
- label: `用本機版`
- description: `以本機版本覆蓋 repo`
- 動作：將本機內容寫入對應的 `claude/<檔案>`（repo 側）

#### 3. 用 VS Code 手動合併（⚠️ 此選項容易被遺漏，務必包含）
- label: `用 VS Code 手動合併`
- description: `開啟 VS Code diff editor，左側本機（唯讀），右側 repo（可編輯），關閉後繼續`
- 動作：
  1. 執行 `code --wait --diff ~/.claude/<檔案> claude/<檔案>`，阻塞等待用戶編輯右側並關閉 tab
  2. 讀取 `claude/<檔案>` 作為合併結果，執行 `node .claude/commands/sync-ai-apply.js --action write-local --file <fileKey>` 寫入本機

#### 4. 略過
- label: `略過`
- description: `保持現狀，跳過此檔案`

處理完所有設定檔後，無條件執行 `cp claude/statusline.sh ~/.claude/statusline.sh`（確保各裝置都有此檔案）。

`<fileKey>` 對應：CLAUDE.md → `claude-md`、settings.json → `settings-json`
...
- **比對腳本**：`.claude/commands/sync-ai-diff.js` 負責所有比對邏輯（LCS diff、JSON deep compare、skills 解析、目錄掃描）
...
- **Agents / Commands 路徑**：使用相對路徑（如 `awesome-claude-code-subagents/backend-developer.md`）；複製前若目標目錄不存在自動 `mkdir -p`
