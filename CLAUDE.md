...
此檔案為 Claude Code 全域指示，由 sync-ai repo 同步管理。
...
```bash
cp $HOME/.claude/CLAUDE.md /Users/barney/code/sync-ai/CLAUDE.md
cp $HOME/.claude/settings.json /Users/barney/code/sync-ai/settings.json
cd /Users/barney/code/sync-ai && git add CLAUDE.md settings.json && git commit -m "chore: sync $(date +%Y-%m-%d)" && git push
```
