...
```bash
node "D:/code/sync-ai/.claude/get-shit-done/bin/gsd-tools.cjs" history-digest
```
...
@D:/code/sync-ai/.claude/get-shit-done/references/thinking-models-planning.md
...
```bash
VALID=$(node "D:/code/sync-ai/.claude/get-shit-done/bin/gsd-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node "D:/code/sync-ai/.claude/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node "D:/code/sync-ai/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
